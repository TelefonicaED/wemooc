...
<div class="container-activity isFeedback">
	...
	<c:if test="${!hasFreeQuestions }">
		<p><liferay-ui:message key="learning-activity.test.your-result" arguments="${calificationType.translate(locale, scoreTry).concat(calificationType.suffix) }" /></p>
	</c:if>
	<c:choose>
		...
		<c:when test="${!hasFreeQuestions }">
			<p><liferay-ui:message key="learning-activity.test.your-result-dont-pass"  arguments="${calificationType.translate(locale, activity.passPuntuation).concat(calificationType.suffix) }" /></p>
		</c:when>
	</c:choose>
