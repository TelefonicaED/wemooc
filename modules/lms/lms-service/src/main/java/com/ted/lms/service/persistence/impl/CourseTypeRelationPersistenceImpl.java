 ...
 * @see com.ted.lms.service.persistence.CourseTypeRelationUtil
 * @generated
 */
@ProviderType
public class CourseTypeRelationPersistenceImpl extends BasePersistenceImpl<CourseTypeRelation>
	...
	public List<CourseTypeRelation> findByCourseTypeIdClassNameId(
		...
		if (retrieveFromCache) {
			list = (List<CourseTypeRelation>)finderCache.getResult(finderPath,
					finderArgs, this);
	...
	public CourseTypeRelation[] findByCourseTypeIdClassNameId_PrevAndNext(
		...
		OrderByComparator<CourseTypeRelation> orderByComparator)
		throws NoSuchCourseTypeRelationException {
		CourseTypeRelation courseTypeRelation = findByPrimaryKey(courseTypeRelationPK);
	...
	public int countByCourseTypeIdClassNameId(long courseTypeId,
		long classNameId) {
		FinderPath finderPath = FINDER_PATH_COUNT_BY_COURSETYPEIDCLASSNAMEID;
	...
	public void cacheResult(CourseTypeRelation courseTypeRelation) {
		entityCache.putResult(CourseTypeRelationModelImpl.ENTITY_CACHE_ENABLED,
	...
	public void clearCache(CourseTypeRelation courseTypeRelation) {
		entityCache.removeResult(CourseTypeRelationModelImpl.ENTITY_CACHE_ENABLED,
			CourseTypeRelationImpl.class, courseTypeRelation.getPrimaryKey());
	...
	public CourseTypeRelation updateImpl(CourseTypeRelation courseTypeRelation) {
		...
		if (!(courseTypeRelation instanceof CourseTypeRelationModelImpl)) {
			...
			if (ProxyUtil.isProxyClass(courseTypeRelation.getClass())) {
				invocationHandler = ProxyUtil.getInvocationHandler(courseTypeRelation);
		...
		try {
			...
			if (courseTypeRelation.isNew()) {
				...
				courseTypeRelation = (CourseTypeRelation)session.merge(courseTypeRelation);
			}
		}
		catch (Exception e) {
			throw processException(e);
		...
		if (!CourseTypeRelationModelImpl.COLUMN_BITMASK_ENABLED) {
			finderCache.clearCache(FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION);
		}
		else
		 if (isNew) {
			...
			finderCache.removeResult(FINDER_PATH_COUNT_BY_COURSETYPEIDCLASSNAMEID,
				args);
			finderCache.removeResult(FINDER_PATH_WITHOUT_PAGINATION_FIND_BY_COURSETYPEIDCLASSNAMEID,
				args);
	...
	/**
	 * Returns the course type relation with the primary key or throws a {@link com.liferay.portal.kernel.exception.NoSuchModelException} if it could not be found.
	 *
	 * @param primaryKey the primary key of the course type relation
	...
	public CourseTypeRelation fetchByPrimaryKey(Serializable primaryKey) {
		Serializable serializable = entityCache.getResult(CourseTypeRelationModelImpl.ENTITY_CACHE_ENABLED,
		...
		CourseTypeRelation courseTypeRelation = (CourseTypeRelation)serializable;
		...
		if (courseTypeRelation == null) {
			...
			try {
				...
				if (courseTypeRelation != null) {
					...
					entityCache.putResult(CourseTypeRelationModelImpl.ENTITY_CACHE_ENABLED,
						CourseTypeRelationImpl.class, primaryKey, nullModel);
				}
			}
			catch (Exception e) {
				entityCache.removeResult(CourseTypeRelationModelImpl.ENTITY_CACHE_ENABLED,
	...
	@Override
	public Map<Serializable, CourseTypeRelation> fetchByPrimaryKeys(
		Set<Serializable> primaryKeys) {
		if (primaryKeys.isEmpty()) {
			return Collections.emptyMap();
		}

		Map<Serializable, CourseTypeRelation> map = new HashMap<Serializable, CourseTypeRelation>();

		for (Serializable primaryKey : primaryKeys) {
			CourseTypeRelation courseTypeRelation = fetchByPrimaryKey(primaryKey);

			if (courseTypeRelation != null) {
				map.put(primaryKey, courseTypeRelation);
			}
		}
	...
	private static final Log _log = LogFactoryUtil.getLog(CourseTypeRelationPersistenceImpl.class);
